...
import { deduceTxType, isFailed, parseTransferSignature } from 'utils/signatures';
...
export default function useParseSignature(tx: any, pubkey: string) {
    ...
    const { loading: parsing, data: parseResult } = useRequest(() => parseTransferSignature(connection, tx, txType, pubkey));
