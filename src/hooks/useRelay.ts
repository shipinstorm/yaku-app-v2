...
export default function useRelay() {
    ...
    const { ethAddress, ethConnected, signMessage, getBlockNumber } = useEthcontext();
    ...
    const processWalletConnection = useCallback(async () => {
        ...
        const requestSolanaSign = async () => {
            ...
            while (true) {
                try {
                    // eslint-disable-next-line
                    const latestBlockDetailsLocal = await connection.getLatestBlockhashAndContext('confirmed');
            ...
            const encodedMessage = new TextEncoder().encode(`Yaku: ${latestBlockDetails?.blockhash}`);
            ...
            try {
                ...
                try {
                    ...
                    showErrorToast(`Error: Something went wrong during signing the verification message.`);
                    return {};
                }
