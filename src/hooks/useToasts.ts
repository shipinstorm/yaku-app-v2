...
export const useToasts = () => {
  ...
  const showTxErrorToast = (error: any, message?: string) => {
    if (error.code === 4001 || error.message.includes("rejected the request")) {
      ...
      error.code === 4100 ||
      error.message.includes("account has not been authorized")
    ) {
      ...
      error.code === -32002 ||
      error.message.includes("resource not available")
    ) {
      ...
      error.code === -32006 ||
      error.message.includes("protocol is not supported")
    ) {
      toast.error(ERROR_MAP.JSON_RPC_NOT_SUPPORT);
    } else if (
      error.code === -32600 ||
      error.message.includes("valid request")
    ) {
      ...
      error.code === -32602 ||
      error.message.includes("method parameter")
    ) {
