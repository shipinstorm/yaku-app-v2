...
export function useRequests() {
    ...
    const [GET_CORAL_USER_PROFILE] = useAuthLazyQuery(queries.GET_CORAL_USER_PROFILE);
    const [GET_WALLET_STATS] = useAuthLazyQuery(queries.GET_WALLET_STATS);
    const [GET_WALLET_ACTIVITIES] = useAuthLazyQuery(queries.GET_WALLET_ACTIVITIES);
    ...
    const [GET_ME_ESCROW_BALANCE] = useAuthLazyQuery(queries.GET_ME_ESCROW_BALANCE);
    ...
    const [SEARCH_PROJECT_BY_NAME] = useAuthLazyQuery(queries.SEARCH_PROJECT_BY_NAME);

    const [GET_MARKETPLACE_SNAPSHOT] = useAuthLazyQuery(queries.GET_MARKETPLACE_SNAPSHOT);
    ...
    const [GET_ME_TRANSACTION_INSTRUCTIONS] = useAuthMutation(mutations.GET_ME_TRANSACTION_INSTRUCTIONS);
    const [GET_ME_WITHDRAW_TRANSACTION_INSTRUCTIONS] = useAuthMutation(mutations.GET_ME_WITHDRAW_TRANSACTION_INSTRUCTIONS);
    ...
    const [GET_ME_BID_TRANSACTION_INSTRUCTIONS] = useAuthMutation(mutations.GET_ME_BID_TRANSACTION_INSTRUCTIONS);
    const [GET_ME_LIST_TRANSACTION_INSTRUCTIONS] = useAuthMutation(mutations.GET_ME_LIST_TRANSACTION_INSTRUCTIONS);
    const [GET_ME_DELIST_TRANSACTION_INSTRUCTIONS] = useAuthMutation(mutations.GET_ME_DELIST_TRANSACTION_INSTRUCTIONS);
    const [GET_ETH_NFT] = useAuthLazyQuery(queries.GET_ETH_NFT);
    const [GET_MP_ACTIVITIES] = useAuthLazyQuery(queries.GET_MP_ACTIVITIES);
    const [GET_LISTING_BY_SYMBOL] = useAuthLazyQuery(queries.GET_LISTING_BY_SYMBOL);
    const [GET_LISTING_BY_MINT] = useAuthLazyQuery(queries.GET_LISTING_BY_MINT);

    const [GET_COLLECTION_HISTORY] = useAuthLazyQuery(queries.GET_COLLECTION_HISTORY);
    ...
    const [GET_ME_TOKEN_OFFER_RECEIVED] = useAuthLazyQuery(queries.GET_ME_TOKEN_OFFER_RECEIVED);
    const [GET_COIN_INFO] = useLazyQuery(queries.GET_COIN_INFO);
    const [GET_COINS_MARKET_CHART] = useAuthLazyQuery(queries.GET_COINS_MARKET_CHART);
    ...
    const [GET_ALL_WORKSPACES_BY_USER] = useLazyQuery(queries.GET_ALL_WORKSPACES_BY_USER);
    ...
    const [CREATE_PROJECT_WALLET] = useAuthMutation(mutations.CREATE_PROJECT_WALLET);
    ...
    const getProjectStats = async (variables: {
        ...
        const { data } = await GET_PROJECTS_STATS({
            variables: { chain, projectIds, onlyListings, field_name, sort_order, page_number, page_size, tags },
            fetchPolicy: 'network-only'
        });
        return data?.getProjectStats || {};
    };

    const getSolProjectData = async (variables: { projectIds: string[]; chain?: string }) => {
    ...
    const getWorkspaceByName = async (variables: { owner: string; name: string }) => {
    ...
    const addUser = async (variables: { id: string; user: { address: string; role: string; claimer?: string } }) => {
    ...
    const setMultisig = async (variables: { id: string; owner: string; multisig: string }) => {
    ...
    const login = async ({ wallet, ethAddress }: { wallet?: string; ethAddress?: string }) => {
    ...
    const signup = async ({ wallet, vanity }: { wallet?: string; vanity?: string }) => {
        if (!wallet && !vanity) {
    ...
    const connectDiscord = async (variables: { address: string; code: string; redirectUri: string }) => {
    ...
    const connectTwitter = async (variables: { address: string; code: string; redirectUri: string }) => {
    ...
    const getTwitterAuthLink = async (variables: { address: string; redirectUri: string }) => {
    ...
    const getMETransactionInstructions = async (variables: { buyer: string; tokenMint: string }) => {
    ...
    const creatDelistTx = async (variables: { sellerAddress: string; tokenAddress: string }) => {
    ...
    const getMEBidTransactionInstructions = async (variables: { buyer: string; tokenMint: string; price: number }) => {
    ...
    const getMEListTransactionInstructions = async (variables: { seller: string; tokenMint: string; price: number }) => {
    ...
    const getMEDelistTransactionInstructions = async (variables: { seller: string; tokenMint: string }) => {
    ...
    const getMEListings = async (variables: { symbol: string; offset?: number; limit?: number }) => {
    ...
    const getTokenChart = async (variables: { coinId: string; params: { vs_currency: string; days: string } }) => {
    ...
    const getWalletStats = async (variables: { condition: { searchAddress: string; timePeriod: string; includeUserRank: boolean } }) => {
    ...
    const getWalletActivities = async (variables: { wallet: any; offset?: number; limit?: number }) => {
    ...
    const getWalletTokens = async (variables: { wallet: string; offset: number; limit: number }) => {
    ...
    const getETHProjectData = async (variables: { projectIds: string[]; tokenId: string }) => {
    ...
    const getWalletProfile = async (wallet: string, chain = 'SOL') => {
        ...
        const { data } = await axios.post(`${hubSvcPath}/user/wallet/profile`, params, {
            headers: {
                'csrf-token': token
            }
        });
    ...
    const getCollectionsByWallet = async (wallet: string) => {
        const token: any = !csrfToken ? await csrf() : csrfToken;
        const { data } = await axios.get(`${hubSvcPath}/wallet/${wallet}/collections`, {
            headers: {
                'csrf-token': token
            }
        });
    ...
    const getInspectorCollection = async (symbol: string, chain: string) => {
        const { data } = await axios.post(`${ethSvcPath}/collection/rank`, { symbol, chain });
        return data;
    };

    const getInspectorCollections = async (chain: string, symbols?: string[]) => {
        const { data } = await axios.post(`${ethSvcPath}/collections/rank`, { symbols, chain });
        return data;
    };

    const getInspectorCollectionMembers = async (symbol: string, chain: string, offset = 0, limit = 20) => {
        const { data } = await axios.post(`${ethSvcPath}/collection/members`, { symbol, chain, limit, offset });
        return data?.members;
    };
    ...
    const getTwitterFollowed = async (username: string, target: string) => {
        const token: any = !csrfToken ? await csrf() : csrfToken;
        const { data } = await axios.get(`${hubSvcPath}/twitter/username/${username}/is-following/target-username/${target}`, {
            headers: {
                'csrf-token': token
            }
        });
    ...
    const createProjectWallet = async (variables: { project: string; wallet: string }) => {
    ...
    const clickWithdraw = async (variables: { project: string; method: string; amount: number }) => {
    ...
    const getEmployees = async (variables: { project: string; employer: string }) => {
    ...
    const deleteClaimer = async (variables: { project: string; name: string; wallet: string; employer: string }) => {
    ...
    const clickClaim = async (variables: { project: string; wallet: string; method: string; employer: string }) => {
    ...
    const initMsTx = async (payload: {
        ...
        const token: any = !csrfToken ? await csrf() : csrfToken;
        const { data } = await axios.post(`${hubSvcPath}/multisig/tx/init`, payload, {
            headers: {
                'csrf-token': token
            }
        });
    ...
    const updateMsTx = async (payload: { id: string; executedAt?: string; signature?: string; error?: string }) => {
        const token: any = !csrfToken ? await csrf() : csrfToken;
        const { data } = await axios.post(`${hubSvcPath}/multisig/tx/update`, payload, {
            headers: {
                'csrf-token': token
            }
        });
            ...
            slides: [
                {
                    ...
                    type: 'video',
                    mobileSrc:
                        'https://s3.amazonaws.com/img.yaku.ai/YakuCombat.mp4',
