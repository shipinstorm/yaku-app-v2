...
import { createTransferInstruction, TOKEN_PROGRAM_ID } from '@solana/spl-token-v2';
...
export function useTransactions() {
    ...
    const transferSol = async (source: string, destination: string, amount: number) => {
    ...
    const transferSPLToken = async (source: string, destination: string, amount: number, tokenMint: string) => {
        if (!wallet) {
