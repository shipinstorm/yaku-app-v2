...
export const numberToString = (value: number | any | undefined, precision = 0) => {
    ...
    const result = round(Number(value?.toFixed(precision)), precision).toLocaleString() ?? '';
...
export const useNumberBoundary = (options: UseNumberInputOptions = {}) => {
    ...
    const [interfaceValue, setInterfaceValue] = useState<string>(formatter(numberToString(defaultValue, precision)));
    ...
    function change(multiplier = 1, params: SpinParams = {}) {
        setInterfaceValue((current) => {
            ...
            return formatter(numberToString(result, digits));
        });
    }
    ...
    return {
        numberValue,
        interfaceValue,
        setInterfaceValue: (val: string) => {
            setInterfaceValue(formatter(val));
        },
        increment,
        decrement
    };
...
function callAllHandlers<T>(...handlers: Array<undefined | ((event: T) => void)>) {
    return (event: T) => {
...
function getError(value: number | undefined, min: number, max: number): NumberInputError | null {
