...
export const loadYakuProgram = (
  ...
  const provider = new anchor.AnchorProvider(
    connection,
    wallet || cloneWindow["solana"],
    anchor.AnchorProvider.defaultOptions()
  );

  const program: any = new anchor.Program(
    YAKU_IDL as anchor.Idl,
    STAKING_PROGRAM_ID,
    provider
  );
  return program;
...
export const SPL_ASSOCIATED_TOKEN_ACCOUNT_PROGRAM_ID =
  new anchor.web3.PublicKey(YAKU_SPL_TOKEN_PROGRAM_ID);
...
export const fetchMetadata = async (
  uri: any,
  mint: any,
  extraKey: string | undefined = undefined,
...
export const getUserStakingAuthority = async (
  ...
  return await anchor.web3.PublicKey.findProgramAddress(
    [config.toBuffer(), encode("auth"), owner.toBuffer()],
    prog.programId
  );
};
...
export const getStakingAccount = async (
  ...
  return await anchor.web3.PublicKey.findProgramAddress(
    [owner.toBuffer(), encode("stake"), mint.toBuffer()],
    prog.programId
  );
};
...
export const getRewardAccount = async (
  ...
  return await anchor.web3.PublicKey.findProgramAddress(
    [config.toBuffer(), encode("reward"), reward.toBuffer()],
    prog.programId
  );
};

export const getStakingAuthority = async (
  config: anchor.web3.PublicKey,
  prog: { programId: anchor.web3.PublicKey }
) => {
  return await anchor.web3.PublicKey.findProgramAddress(
    [config.toBuffer(), encode("auth")],
    prog.programId
  );
};
...
export const getAllPDAs = async (
  ...
  const [authority, authBump] = await getUserStakingAuthority(
    config,
    owner,
    program
  );
  ...
  const [stakeAccount, stakeBump] = await getStakingAccount(
    owner,
    nft,
    program
  );
  const [rewardAccount, rewardBump] = await getRewardAccount(
    config,
    reward,
    program
  );
  const [stakingAuthority, stakingAuthorityBump] = await getStakingAuthority(
    config,
    program
  );
...
export const getNFTTokenAccount = async (
  ...
  try {
    ...
    let _listOfTokens: any = await connection.getParsedProgramAccounts(
      TOKEN_PUBKEY,
...
export const stakeYakuNft = async (
  program: anchor.Program<Staking>,
  wallet: WalletContextState,
  mint: anchor.web3.PublicKey
) => {
  ...
  const {
    ...
    stakeAccount,
    stakeBump,
  } = await getAllPDAs(program, wallet.publicKey, mint);
  ...
  txn.add(
    program.instruction.stakeRarity(
      authBump,
      stakeBump,
      tokenBump!,
      new anchor.BN(index),
...
export const stakeYakuPNft = async (
  program: anchor.Program<Staking>,
  wallet: WalletContextState,
  mint: anchor.web3.PublicKey,
  priorityRate = 100
  ...
  if (!wallet?.publicKey) return;
  const { authority, authBump, stakeAccount, stakeBump } = await getAllPDAs(
    program,
    wallet.publicKey,
    mint
  );
  ...
  const accountData = await fetchAccountDataById(program.provider.connection, [
    mintMetadataId,
    ownerTokenRecord,
  ]);
  ...
  txn
    ...
    .add(priorityIx)
    .add(
      program.instruction.stakeRarityPnft(
        authBump,
        stakeBump,
        new anchor.BN(index),
        new anchor.BN(reward),
...
export const unstakeYakuNft = async (
  program: anchor.Program<Staking>,
  wallet: WalletContextState,
  mint: anchor.web3.PublicKey
) => {
  ...
  const stakeAccountState = await program.account.stakingAccount.fetch(
    stakeAccount
  );
  const { interval } = await getStakingState(program);
  const computedReward =
    ((Math.floor(Date.now() / 1000) - stakeAccountState.lastClaim.toNumber()) /
      interval) *
    stakeAccountState.amount.toNumber();
  if (computedReward > 0) {
    txn.add(
...
export const unstakeYakuPNft = async (
  program: anchor.Program<Staking>,
  wallet: WalletContextState,
  mint: anchor.web3.PublicKey,
  priorityRate = 100
...
export const unstakeYakuNonCust = async (
  program: anchor.Program<Staking>,
  wallet: WalletContextState,
  mint: anchor.web3.PublicKey,
  priorityRate = 100
  ...
  if (!wallet?.publicKey) return;
  const { authority, stakeAccount, rewardAccount, stakingAuthority } =
    await getAllPDAs(program, wallet.publicKey, mint);
...
export const claimYakuReward = async (
  program: anchor.Program<Staking>,
  wallet: WalletContextState,
  mint: anchor.web3.PublicKey,
  priorityRate = 100
  ...
  if (!wallet?.publicKey) return;
  const { stakingAuthority, stakeAccount, rewardAccount } = await getAllPDAs(
    program,
    wallet.publicKey,
    mint
  );
...
export const stakeNftV2Multiple = async (
  ...
  for (let i = 0; i < mintList.length; i++) {
    ...
    const { stakeAccount, stakeBump, authority, authBump } = await getAllPDAs(
      program,
      wallet.publicKey,
      mint
    );
    ...
    const fromAccount = ownerTokenAccountId;
    const ownerTokenRecord = findTokenRecordId(mint, ownerTokenAccountId);
    const accountData = await fetchAccountDataById(
      program.provider.connection,
      [mintMetadataId, ownerTokenRecord]
    );
    const metadataAccountInfo = accountData[mintMetadataId.toString()];
    const mintMetadata = metadataAccountInfo
      ? Metadata.deserialize(metadataAccountInfo.data)[0]
  ...
  for (let i = 0; i < mintList.length; i++) {
    ...
    const computedReward =
      ((Math.floor(Date.now() / 1000) -
        stakeAccountState.lastClaim.toNumber()) /
        interval) *
      stakeAccountState.amount.toNumber();
    if (computedReward > 0) {
      txn.add(
...
export const claimRewardV2Multiple = async (
  ...
  for (let i = 0; i < mintList.length; i++) {
    const mint = mintList[i];
    const { stakeAccount, rewardAccount, stakingAuthority } = await getAllPDAs(
      program,
      wallet.publicKey,
      mint
    );
