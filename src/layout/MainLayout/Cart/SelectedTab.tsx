...
const SelectedTab = (props: {
  ...
  const { selectedNfts, buyNowMulti, onDeSelect, solBalance, ethBalance } =
    props;
  ...
  const solTokens = useMemo(
    () =>
      selectedNfts.reduce(
        (result: SelectedItemType[], item: SelectedItemType) => {
          if (item.chain === "SOL") {
            result.push(item);
          }
          return result;
  ...
  const { loading: wsLoading, data: workspaces } = useRequest(
    () => getAllWorkspacesByUser({ user: wallet?.publicKey?.toBase58()! }),
    {
  ...
  const msWorkspaces: any[] = useMemo(
    () => workspaces?.filter((workspace: any) => !!workspace.multisig) ?? [],
    [workspaces]
  );
  ...
  const workspaceOwner: string = useMemo(() => {
    if (msWorkspaces.length > 0 && workspaceId !== "") {
      ...
      const msWsIndex = msWorkspaces.findIndex(
        (msWs) => msWs._id === workspaceId
      );
      if (msWsIndex > -1) {
      ...
      for (const solNFT of solTokens) {
        try {
          ...
          if (await handleBuyNFT(solNFT.tokenAddress)) {
            showSuccessToast(
              `Successfully created a multisig transaction to buy '${solNFT.name}`
            );
            msData!.transactionIndex += 1;
            onDeSelect(solNFT.tokenAddress, solNFT.contractAddress);
          } else {
            showErrorToast(
              `Failed to create a multisig transaction to buy '${solNFT.name}`
            );
          }
        } catch (e) {
          console.error(e);
          showErrorToast(
            `Failed to create a transaction to buy '${solNFT.name}'`
          );
        }
      }
          ...
          selectedNfts.map((item: any, key: any) => (
            <div className="selected-nft" key={key}>
              ...
              <div className="nft-price">
                ...
                <IconButton
                  onClick={() =>
                    onDeSelect(item.tokenAddress, item.contractAddress)
                  }
                  style={{
                    position: "absolute",
                    left: 22,
            ...
            <div className="w-full">
              ...
              <Select
                ...
                onChange={(e: any) => {
                  ...
                  if ((e.target.value as WalletType) === WalletType.MULTISIG) {
                    /* eslint-disable-next-line no-underscore-dangle */
                    setWorkspaceId(
                      msWorkspaces.length > 0 ? msWorkspaces[0]._id : ""
                    );
                  }
                }}
                label="Age"
              ...
              >
                {walletTypes!.map((category, index) => (
                  <MenuItem
                    key={`wallet-category-item-${index}`}
