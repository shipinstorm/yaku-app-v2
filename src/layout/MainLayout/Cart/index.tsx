...
export default function Cart() {
  ...
  const buyNowMultiInETH = async (tokensInfo: SelectedItemType[]) => {
    if (!ethConnected || !ethAddress) {
      ...
      try {
        startLoading();
        const tokens = tokensInfo.map(
          (token) => `${token.contractAddress}:${token.tokenAddress}`
        );
  ...
  const onDeSelect = (tokenAddress: string, contractAddress: string) => {
    ...
    if (selected && selected.length !== 0) {
      ...
      for (let i = 0; i < selected.length; i += 1) {
        if (
          selected[i].tokenAddress === tokenAddress &&
          selected[i].contractAddress === contractAddress
        ) {
          index = i;
        }
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <SwitchList
            handleTabChange={handleMultiTabChange}
            matchUpMd
            tabIdx={multiTabIdx}
            tabList={cartTabList}
          />
