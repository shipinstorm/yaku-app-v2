  ...
  const getSolTokensByOwner = async (tokenAddresses: string[]) => {
    ...
    collectionsList &&
      map(collectionsList[Number(collectionId)].items, (tokens) => {
        const item: any = newData.market_place_snapshots.find(
        ...
        if (item && item.lowest_listing_mpa && item.lowest_listing_mpa.price) {
          listed.push({
            ...
            marketplace_program_id:
              item.lowest_listing_mpa.marketplace_program_id,
          });
        } else {
          unListed.push({
            ...tokens,
      ...
      if (listedTokens && listedTokens.length !== 0) {
        for (const item of listedTokens) {
          if (Number(item.price) !== Number(item.newPrice)) {
            const transaction = await callListTx(
              item.mint,
              item.newPrice,
              item.marketplace_program_id
            );
            if (transaction) {
  ...
  return (
    <>
      ...
      >
        <FormControl
          size="small"
          sx={{ mt: 2, mb: 2, pr: 2, pl: 2, width: "100%" }}
        >
            ...
            >
              ...
              <FormControl variant="outlined" sx={{ maxWidth: "100px" }}>
                <OutlinedInput
                  ...
                  sx={{
                    padding: "0px",
                    "& input": { padding: "10px 0px 10px 10px !important" },
                  }}
                />
              </FormControl>
            </Box>
