...
import { Autocomplete, Avatar, Box, Card, CircularProgress, Grid, InputAdornment, Paper, Popper } from '@mui/material';
...
const SearchSection = () => {
    ...
    const handleSearch = async (keyword: string) => {
        ...
        try {
            ...
            if (names && names.length !== 0) {
                ...
                newOptions = [domainOptions, ...filter(newOptions, ({ type }) => type !== 'domain')];
                setOptions(newOptions);
            }
            // }
        } catch (error) {
    ...
    const handleNavigate = ({ type, project_id, mint }: any) => {
        switch (type) {
            case 'coin':
                ...
                break;
            case 'collection':
                navigate(`/explore/collection/SOL/${project_id}`);
                dispatch(activeItem(['collection']));
                break;
            case 'nft':
                navigate(`/explore/collection/SOL/${project_id}/${mint}`);
                dispatch(activeItem(['collection']));
                break;
            case 'account':
    ...
    return (
        <>
            <Box className="bg-surface text-terciary" sx={{ display: { xs: 'block', md: 'none' } }}>
