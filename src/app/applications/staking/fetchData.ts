...
export const getUnstakedNfts = async ({
  ...
  const list: Array<any> = await Promise.mapSeries(
    nftsList,
    async (item: NFTType) => {
      try {
        const creators = get(item, "data.creators", []);
        if (
          !!get(creators, "[0].verified") &&
          YAKU_COLLECTION_CREATORS.includes(get(creators, "[0].address") || "")
        ) {
...
export const getYakuStakedNfts = async ({
  ...
  const staked = await getStakedNFTMintList(
    connection,
    program,
    wallet,
    otherWallet
...
export const getUserPoolData = async (props: {
  ...
  return {
    staked: uniqBy(
      filter(
        staked,
        (itm) => !!itm && itm.mint !== "11111111111111111111111111111111"
      ),
      "mint"
    ),
    count,
...
export const getTokenDistribution = async (connection: Connection) => {
  ...
  let yakuMeta: any = {};
  try {
    ({ value: yakuMeta } = await connection.getTokenSupply(
      new PublicKey(STAKING_REWARD_MINT)
    ));
  } catch (error: any) {
