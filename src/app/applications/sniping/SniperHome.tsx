...
const SniperHome = () => {
  ...
  const [snipedIdentifier, setSnipedIdentifier] =
    useRecoilState(snipedIdentifierAtom);
  ...
  const getSnipeTx = async (MEData: any, walletAddress: string) => {
    try {
      ...
      const { data } = await axios.post(
        `https://proxy.yaku.ai/api/me/tx/snipe`,
        {
          mint: tokenMint,
          buyer: walletAddress,
          seller,
  ...
  const getTransactionWithRetries = async (conn: web3.Connection, res: any) => {
    ...
    while (tries < 100 && signature === null) {
      ...
      await delay(100);
      try {
        const getTransResp = await conn.getTransaction(res, {
        ...
        if (
          typeof getTransResp === "object" &&
          getTransResp?.meta?.err === null
        ) {
          showInfoToast(
            <a
  ...
  const buyNow = async (MEData: any, identifier: never, tryOnRpc = true) => {
    if (wallet.publicKey && !snipedIdentifier.includes(identifier)) {
      ...
      try {
        let response;
        response = await getBuyInstructions(
          MEData,
        ...
        if (Object.keys(response).length > 0) {
          ...
          if (tryOnRpc) {
                  ...
                  fetchMiddleware: (url, options, fetch) => {
                    if (options) {
                      set(
                        options,
                        "headers",
                        omit(options.headers!, ["solana-client"])
                      );
                      fetch(url, options);
                    }
                  },
            ...
            const sigantures = await Promise.map(conns, (con) =>
              wallet
                .sendTransaction(transaction, con, { maxRetries: 2 })
                .then(async (res) => {
                  await con.confirmTransaction(res);
                  const signature = await getTransactionWithRetries(con, res);
                  if (signature !== -1) {
                    setSnipedIdentifier(
                      [identifier].concat(cloneDeep(snipedIdentifier))
                    );
                  }
                  return signature;
                })
