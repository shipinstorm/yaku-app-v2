...
import { cancelMsTx, confirmMsTx, rejectMsTx, executeMsTx as execMsTx, MS_TRANSACTION_REFRESH_TIMEOUT } from 'utils/ms';
...
const TransactionCard: React.FC<Props> = ({ tx, threshold, owners, refresh, setRefreshingState, setTempData }: Props) => {
    ...
    const canTakeAction = useMemo(() => {
        if (!!owners && wallet.connected && wallet.publicKey) {
            return owners.map((item) => item.toString()).includes(wallet.publicKey.toString());
        }
        return false;
    ...
    return (
        <div className="transaction-card py-2 my-2">
            <div className="px-2 py-2">
                <StatusBar tx={tx} isPanelShown={panelShown} onToggle={() => setPanelShown((oldValue) => !oldValue)} />
            </div>
            {panelShown && (
                <>
                        ...
                        <Grid item xs={6} className="px-2">
                            <div className="info-panel h-100">
                                ...
                                <div className="flex justify-between pt-1 mt-1">
                                    ...
                                    <span>{tx.executedAt ? new Date(tx.executedAt).toDateString() : 'n/a'}</span>
                                </div>
                                <div className="flex justify-between pt-1 mt-1">
                                    <span>Transaction Link</span>
                                    <span>
                                        {tx.signature ? (
                                            <PubkeyLink customClass="underline" pubkey={tx.signature} prefixSize={10} type="tx" />
                                        ) : (
                                            'n/a'
                                        )}
                                    </span>
                        ...
                        <Grid item xs={6} className="px-2">
                            <div className="result-panel h-100">
                                ...
                                <Grid container spacing={1}>
                                    <Grid item xs={4}>
                                        <div className="count-info confirmed-count py-1 flex justify-center">{tx.approved.length}</div>
                                        <div className="flex justify-center">confirmed</div>
                                    </Grid>
                                    <Grid item xs={4}>
                                        <div className="count-info rejected-count py-1 flex justify-center">
                                            {threshold && tx.approved.length >= threshold ? tx.cancelled.length : tx.rejected.length}
                                        </div>
                                        <div className="flex justify-center">
                                            {threshold && tx.approved.length >= threshold ? 'Cancelled' : 'Rejected'}
                                        </div>
                                    </Grid>
                                    <Grid item xs={4}>
                                        <div className="count-info threshold-count py-1 flex justify-center">
                                            {threshold && owners ? `${threshold}/${owners.length}` : 'n/a'}
                                        </div>
                                            ...
                                            <>
                                                <Grid item xs={6}>
                                                    <Button
                                                        ...
                                                        disabled={!canCancel || !canTakeAction || cancelLoading}
                                                    >
                                                        {cancelLoading ? (
                                                            <LoadingSpinner />
                                                        ) : (
                                                            <span className={canCancel && canTakeAction ? '' : 'text-dummy'}>Cancel</span>
                                                        )}
                                                    </Button>
                                                </Grid>
                                                <Grid item xs={6}>
                                                    ...
                                                    >
                                                        {executeLoading ? (
                                                            ...
                                                            <span className={canTakeAction ? '' : 'text-dummy'}>Execute</span>
                                                        )}
                                                    </Button>
                                                </Grid>
                                            </>
                                            ...
                                            <>
                                                <Grid item xs={6}>
                                                    <Button
                                                        ...
                                                        disabled={!canReject || !canTakeAction || rejectLoading}
                                                    >
                                                        {rejectLoading ? (
                                                            <LoadingSpinner />
                                                        ) : (
                                                            <span className={canReject && canTakeAction ? '' : 'text-dummy'}>Reject</span>
                                                        )}
                                                    </Button>
                                                </Grid>
                                                <Grid item xs={6}>
                                                    <Button
                                                        ...
                                                        disabled={!canConfirm || !canTakeAction || confirmLoading}
                                                    >
                                                        {confirmLoading ? (
                                                            <LoadingSpinner />
                                                        ) : (
                                                            <span className={canTakeAction && canConfirm ? '' : 'text-dummy'}>Confirm</span>
                                                        )}
                                                    </Button>
                                                </Grid>
                                            </>
