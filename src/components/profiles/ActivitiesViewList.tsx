...
const ActivitiesViewList = ({
  ...
  const [cacheSocialWallets, setCacheSocialWallets] = useLocalStorage(
    "socialWallets",
    {}
  );
    ...
    () => {
      ...
      each(activityData, (activity: any) => {
        if (
          ...
          activity &&
          activity.buyer &&
          !wallets.includes(activity.buyer)
        ) {
        ...
        if (
          ...
          activity &&
          activity.seller &&
          !wallets.includes(activity.seller)
        ) {
    ...
    <section className="activity-box bg-elevation1 flex-shrink-0 rounded-3xl shadow-sm">
      <div className="h-full p-4 overflow-auto" id="activities-view-list">
        ...
        <InfiniteScroll
          ...
          loader={
            <LinearProgress key="loadMore" sx={{ mt: 2 }} color="secondary" />
          }
          useWindow={false}
        >
          {isMyActivity
            ...
            : map(
                activityData,
                ({ meta_data_img, name, market_place_state }, idx) => (
                  <ActivityCard
                    idx={idx}
                    name={name}
                    img={meta_data_img}
          ...
          {isLoading &&
            map(Array(count), (i: any) => <ActivityCardSkeleton id={i} />)}
        </InfiniteScroll>
      </div>
    </section>
  );
