...
const FollowSection = ({ title, list, loading }: any) => {
    ...
    const getCount = (obj: any) => (formatAmount(isArray(obj) ? obj?.length || 0 : obj || 0, 0) || 0).toString();
        ...
        <>
            <div>
                <button
                    ...
                    disabled={loading}
                >
                    {loading ? <Skeleton variant="text" className="mx-auto" width={30} /> : getCount(list)}
                </button>
                <p className="text-terciary text-xs font-medium">{title}</p>
            </div>

            <Dialog open={showDialog} onClose={() => setShowDialog(false)} fullWidth maxWidth="xs">
                <DialogTitle>
                    ...
                    {list && isArray(list) && list.length > 0 ? (
                        list.map((el: any, idx: any) => (
                            <div key={idx} className="flex gap-2">
                                <p className="w-1/2">{el.vanity || el?.discord?.name || shortenAddress(el.wallet)}</p>
