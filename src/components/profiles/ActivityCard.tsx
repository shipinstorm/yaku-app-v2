...
const ActivityCard = (props: any) => {
  ...
  const buyerSocialWallet =
    buyer && find(socialWallets, (social) => social.wallet === buyer);
  const sellerSocialWallet =
    seller && find(socialWallets, (social) => social.wallet === seller);
  const { showLoadingToast, dismissToast } = useToasts();

  const getTxTypeLabel = (txType: string) => {
    if (
      (txType === "buyNow" || txType === "TRANSACTION" || txType === "sale") &&
      buyer === wallet
    ) {
  ...
  const gotoCollectionPage = async () => {
    if (mint) {
      ...
      router.push(
        `/explore/collection/SOL/${
          isPlainObject(collection) ? collection.name : collection
        }`
      );
      dismissToast();
    }
    ...
    >
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <div className="flex gap-1 items-center">
              <p className="text-terciary text-sm font-medium whitespace-nowrap">
                {getTxTypeLabel(type).label}
              </p>
              <>
            ...
            <img
              src={
                chain === "SOL"
                  ...
                  : "/images/icons/etherscan-logo-circle.svg"
              }
              alt=""
              width={16}
              height={16}
      ...
      <div className="flex items-center justify-between mt-3 pt-2 rounded-xl shadow-none">
        <h4 className="font-bold">
          {Number(price) > 0 ? `${Big(price).round(3).toLocaleString()} ◎` : ""}
        </h4>
