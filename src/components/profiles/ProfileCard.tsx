...
import { IMAGE_PROXY_BANNER, DEFAULT_BANNER, LOGO_BLACK } from 'config/config';
    ...
    return (
        <section className="profile-box bg-surface mb-5 p-0 card overflow-hidden">
            <div className="banner bg-high-bg relative">
                <img className="w-full h-full object-cover" src={profileBanner || `${IMAGE_PROXY_BANNER}${DEFAULT_BANNER}`} alt="banner" />
                {isEdit && (
            ...
            <div className="p-4 text-center">
                <div className="flex justify-center">
                    {isShowFollowers && <FollowSection title={followersTitle} list={followers} loading={loading} />}

                    <div className="avatar relative z-10 flex-shrink-0 -mt-14 mx-2 rounded-3xl shadow-sm overflow-hidden">
                        {!loading ? (
                            <img className="icon-lg w-full h-full object-cover" src={profileAvatar || LOGO_BLACK} alt="avatar" />
                        ) : (
                    ...
                    {isShowFollowers && <FollowSection title={followingsTitle} list={followings} loading={loading} />}
                </div>
                ...
                {isEdit ? (
                    ...
                    <>
                        <div className="p-4 pt-6 border-elevation2">
                            <h3 className="text-primary mb-2 text-base font-bold">{profileName || ''}</h3>
                            <p className="text-terciary font-medium text-sm mb-2">{profileBio || ''}</p>
                            {!showAsCell &&
                                textContent &&
                                    ...
                                    (idx: any) =>
                                        idx && (
                                            <div className="text-terciary flex items-center justify-center gap-1 text-sm font-medium mt-3">
                                                ...
                                                {idx.value ? <p className="text-terciary font-medium text-sm">{idx.value}</p> : <></>}
                                            </div>
                                        )
                                )}
                            {showAsCell && (
                                <div className="grid grid-cols-2 gap-2">
                                    {!loading ? (
                                        map(textContent, (item: any, idx: number) => <WalletCell key={`cell-${idx}`} {...item} asCol />)
                                    ) : (
                                        <Skeleton className="col-span-2" width="100%" height={32} variant="rounded" />
                                    )}
                                </div>
                            )}
                                                ...
                                                >
                                                    {content.link ? (
                                                        <a href={content.link} target="_blank" rel="noreferrer">
                                                            <img src={content.img} alt={content.alt} className={content.class} />
                                                        </a>
                                                    ) : (
                        ...
                        {isEditable && (
                            <button type="button" className="button-main mt-3" onClick={() => setEdit(true)}>
                                Edit Profile
                            </button>
                        )}
                    </>
