...
import { getGlobalData, getTokenDistribution, getUserPoolData } from '@/app/applications/staking/fetchData';
...
export const StakedProvider: FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const { showErrorToast, showTxErrorToast, showLoadingToast, dismissToast } = useToasts();
    ...
    const getAllYakuNfts = async () => {
        ...
        try {
            ...
            } = await axios.post('https://nft.yaku.ai/api/yaku/wallet', { wallet: wallet.publicKey?.toBase58() });
            setNftList(unstaked);
    ...
    const updateContext = () => {
        ...
        getAllYakuNfts();
        if (!tokenDistributed) {
            getTokenDistribution(connection).then((tokenDist) => setTokenDistributed(tokenDist));
        }
    };
