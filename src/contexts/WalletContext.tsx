...
export const WalletHandlerProvider: FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const { getSubwallet, getAllWorkspaces, getAllWorkspacesByUser, login, connectDiscord, connectTwitter } = useRequests();
    // Metamask context
    const { ethAddress, ethConnected } = useEthcontext();
    const [workspace, setWorkspace, removeWorkspace] = useLocalStorage('workspace', {});
        ...
        if (publicKey && !auth.isAttempting) {
            try {
                ...
                if (data?.token) {
                    ...
                    if (searchParams && searchParams.get('code') && (auth.token || data.token)) {
    ...
    const attemptLoginEth = async (slient = true) => {
        if (ethAddress && !auth.isAttempting) {
            try {
                ...
                auth.attempting(true);
                const data = await login({ ethAddress, wallet: auth.myPublic || publicKey?.toBase58() });
                if (data?.token) {
    ...
    useEffect(() => {
        ...
        console.debug({ publicKey, connected, token: auth.token, isAttempting: auth.isAttempting });
    ...
    const showLoginDialog = (requireSign = false, allowDismiss = true, hideEth = false) => {
    ...
    return (
        <WalletsContext.Provider
            value={{ checkWallet, handleClose, attemptLoginEth, attemptLogin, showLoginDialog, workspaces, fetchWorkpacesCount }}
        >
            {children}
            <Dialog
                open={open}
                ...
                keepMounted
            >
                <WalletLogin open={open} dismiss={canDismiss && handleClose} requireSign={needSign} hideEthButton={hideEthButton} />
            </Dialog>
        </WalletsContext.Provider>
    );
};
...
export const WalletContext: FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const [selectedEndpt, setSelectedEndpt] = useState(USE_QUIKNODE ? DEFAULT_RPC : clusterApiUrl(network));
