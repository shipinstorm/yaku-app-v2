...
export const deduceTxType = (
  logMessages: string[],
  message: { instructions: any[] }
) => {
  ...
  if (
    ...
    message.instructions[0]?.parsed?.type === "create"
  ) {
...
export const includeType = (ixs: any[], type: string): boolean =>
  ixs.findIndex((idx: any) => idx.parsed?.type === type) > -1;

export const isFailed = (message: any): boolean => !!message.err;

export const isExecuted = (logMessages: any[]) =>
  logMessages.join(",").includes("ExecuteTransaction");

export const parseTransferSignature = async (
  ...
  if (txType.toLowerCase() === "transfer") {
    ...
    if (ixIndex > -1) {
      ...
      if (curIx.programId === SystemProgram.programId.toBase58()) {
        ...
        const { name, symbol, image } = await getMintInformation(
          connection,
          mint
        );
      ...
      const { authority, source, destination, tokenAmount, mint } =
        curIx.parsed.info;
    ...
    ixIndex = tx.message.instructions.findIndex(
  ...
  if (txType.toLowerCase() === "mint") {
    if (isExecuted(tx.logMessages)) {
      // find mint address
      const ixIndex = tx.message.instructions.findIndex(
        (ix: any) => ix.programId === PROGRAM_ID.toBase58()
      );
      if (ixIndex > -1) {
