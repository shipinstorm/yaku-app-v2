...
import { sendSignedTransaction, sendTransactionWithRetry } from './transactions';
import { createSolTransferInstruction, getATA, getLamportsByAddress, getTokensOfOwner, TokenData } from './web3';
import NativeSolIcon from '../assets/images/solana-icon.png';
import EmptyNFTIcon from '../assets/images/empty-nft.png';
import {
    NATIVE_MINT,
...
export const createMultisig = async ({
    ...
    const transferInstruction = createSolTransferInstruction(wallet.publicKey!, vaultPDA, 1000000);
    txIns.push(transferInstruction);

    const { txid } = await sendTransactionWithRetry(connection, wallet, txIns, []);
    console.debug('Create multisig transaction signature:', txid);

    return msPDA;
};

export const getMultisig = async (connection: Connection, msId: string): Promise<MsArgs> => {
...
export const getVault = async (connection: Connection, multisig: string, mode = 'metaplex', authorityIndex = 1): Promise<VaultInfo> => {
    const msPubkey = new PublicKey(multisig);
    const [vaultPDA] = getAuthorityPDA(msPubkey!, new BN(authorityIndex));
    const lamports = (await getLamportsByAddress(connection, vaultPDA)) / LAMPORTS_PER_SOL;
...
export const getMsTransaction = async (connection: Connection, multisig: string): Promise<MsTransactionArgs[]> => {
    ...
    const txs = transactionAccounts.map((item) => MsTransaction.deserialize(item.account.data as Buffer, 0)[0] as MsTransactionArgs);

    return txs;
};

export const getRecentTxs = async (pubkey: PublicKey | null): Promise<any[]> => {
    try {
        if (!pubkey) {
            return [];
        }
        const { data: executedTxs } = await axios.post('https://nft.yaku.ai/api/tx/wallet', {
...
export const getMsTxInstructions = async (connection: Connection, tx: PublicKey): Promise<MsInstructionArgs[]> => {
    const txAccountInfo = await connection.getAccountInfo(tx, 'confirmed');
    if (txAccountInfo) {
        const txData = MsTransaction.deserialize(txAccountInfo.data as Buffer, 0)[0] as MsTransactionArgs;
        ...
        const ixList = await Promise.all(
            Array.from(Array(txData.instructionIndex).keys()).map(async (ixIndex) => {
                ...
                if (ixAccountInfo) {
                    const ixData = MsInstruction.deserialize(ixAccountInfo.data as Buffer, 0)[0] as MsInstructionArgs;
                    return ixData;
                }
                return null;
            })
...
export const getMultipleMsTxInstructions = async (connection: Connection, txs: PublicKey[]): Promise<Map<string, MsInstructionArgs[]>> => {
...
export const getAssetChoice = async (nftItem: TokenData): Promise<AssetChoice> => {
    const dataURI = nftItem?.metaplexData?.data.data.uri;
    let tokenIcon = EmptyNFTIcon;
    ...
    return {
        ...
        decimals: nftItem.tokenAccount?.account.data.parsed.info.tokenAmount.decimals
    } as AssetChoice;
};

export const getVaultAssets = async (vault: VaultInfo | undefined, isSOLIncluded = true): Promise<AssetChoice[]> => {
    ...
    if (vault && vault.nftList.length > 0) {
        const vaultAssets = await Promise.all(vault.nftList.map(async (nftItem) => getAssetChoice(nftItem)));
        return assetList.concat(vaultAssets);
    }
    return assetList;
};

export const getVaultNFTInfo = async (vault: VaultInfo | undefined): Promise<AssetInfo[]> => {
    if (vault && vault.nftList.length > 0) {
        const nftInfos = await Promise.all(
            vault.nftList.map(async (nftItem) => {
                if (nftItem.metaplexData?.data) {
                    ...
                    if (dataURI) {
                        const tokenInfo = (await (await fetch(dataURI)).json()) as AssetInfo;
                        tokenInfo.pubkey = nftItem.tokenAccount?.account.data.parsed.info.mint;
                        return tokenInfo;
                    }
                }
                return null;
...
export const getWalletAssets = async (
    ...
    if (pubkey) {
        ...
        if (nftList.length > 0) {
            ...
            return assetList.concat(filter(vaultAssets, (asset) => !!asset) as AssetChoice[]);
        }
    }
    return assetList;
};

export const getVaultBalance = (vault: VaultInfo | undefined, solPrice: number): string =>
    vault ? (vault.lamports * solPrice).toFixed(2) : '0';
...
export const depositNFTs = async ({
    ...
    for (const asset of assets) {
        ...
        const walletTAs = (await connection.getParsedTokenAccountsByOwner(wallet.publicKey!, { mint })).value;
        if (walletTAs.length === 0) {
            throw new Error('The wallet does not have the NFT');
        }
        const walletTAIndex = walletTAs.findIndex((walletTAItem) => walletTAItem.account.data.parsed.info.tokenAmount.amount > 0);
        if (walletTAIndex > -1) {
        ...
        const vaultTAs = (await connection.getParsedTokenAccountsByOwner(vault.pubkey, { mint })).value;
        if (vaultTAs.length === 0) {
            vaultTA = getATA(vault.pubkey, mint);
            instructions.push(createAssociatedTokenAccountInstruction(wallet.publicKey!, vaultTA, vault.pubkey, mint));
        } else {
    ...
    const { txid } = await sendTransactionWithRetry(connection, wallet, instructions, []);
...
export const depositAsset = async ({
    ...
    if (asset === NATIVE_MINT.toString()) {
        instructions = [createSolTransferInstruction(wallet.publicKey, vault.pubkey, amount * LAMPORTS_PER_SOL)];
    } else {
        // find token account of wallet
        let walletTA: PublicKey;
        const walletTAs = (await connection.getParsedTokenAccountsByOwner(wallet.publicKey, { mint })).value;
        ...
        // transfer
        instructions.push(createTransferCheckedInstruction(walletTA, mint, vaultTA, wallet.publicKey, amount * 10 ** decimals, decimals));
    }
...
export const executeMsTx = async ({
    ...
    const instruction = await getExecuteInstruction(connection, msPDA, tx.instructionIndex, transactionPDA, wallet.publicKey);

    const { txid } = await sendTransactionWithRetry(connection, wallet, [instruction], [], 1400000);
...
export const buyNowME = async ({
    ...
    try {
        ...
        for (const signedTransaction of signedTxs) {
            ...
            console.debug(`Signature of the ${executedTxIndex + 1}th transaction: ${signature.txid}`);
            executedTxIndex += 1;
        }
    } catch (e) {
        console.error(e);
