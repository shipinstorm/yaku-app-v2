...
import { createAssociatedTokenAccountInstruction, createTransferCheckedInstruction, createTransferInstruction } from '@solana/spl-token-v2';
import { AccountMeta, Connection, LAMPORTS_PER_SOL, PublicKey, Transaction, TransactionInstruction } from '@solana/web3.js';
...
export const getMsPDA = (createKey: PublicKey): [PublicKey, number] =>
    PublicKey.findProgramAddressSync([Buffer.from('squad'), createKey.toBuffer(), Buffer.from('multisig')], PROGRAM_ID);

export const getTxPDA = (msPDA: PublicKey, txIndex: BN): [PublicKey, number] =>
    PublicKey.findProgramAddressSync(
        [Buffer.from('squad'), msPDA.toBuffer(), txIndex.toArrayLike(Buffer, 'le', 4), Buffer.from('transaction')],
        PROGRAM_ID
    );

export const getIxPDA = (txPDA: PublicKey, IxIndex: BN): [PublicKey, number] =>
    PublicKey.findProgramAddressSync(
        [Buffer.from('squad'), txPDA.toBuffer(), IxIndex.toArrayLike(Buffer, 'le', 1), Buffer.from('instruction')],
        PROGRAM_ID
    );

export const getAuthorityPDA = (multisig: PublicKey, authorityIndex: BN): [PublicKey, number] =>
    PublicKey.findProgramAddressSync(
        [Buffer.from('squad'), multisig.toBuffer(), authorityIndex.toArrayLike(Buffer, 'le', 4), Buffer.from('authority')],
        PROGRAM_ID
    );

export const getTxPDAFromString = (multisig: string, txIndex: number): string => {
...
export const getSendNFTInstructions = async ({
    ...
    for (const asset of assets) {
        ...
        if (vaultTA) {
            ...
            const recipientTAs = (await connection.getParsedTokenAccountsByOwner(recipient, { mint })).value;
            if (recipientTAs.length === 0) {
                recipientTA = getATA(recipient, mint);
                instructions.push(createAssociatedTokenAccountInstruction(payer, recipientTA, recipient, mint));
            } else {
                recipientTA = recipientTAs[0].pubkey;
            }

            instructions.push(
                createTransferInstruction(vaultTA.pubkey, recipientTA, vault, vaultTA.account.data.parsed.info.tokenAmount.amount)
            );
        }
    }
...
export const getSendInstructions = async ({
    ...
    if (mint.toString() === NATIVE_MINT.toString()) {
        incomingIx = createSolTransferInstruction(vault, recipient, amount * LAMPORTS_PER_SOL);
    } else {
        incomingIx = createTransferCheckedInstruction(vaultTA!, mint, recipientTA, vault, amount * 10 ** decimals, decimals);
    }
    instructions.push(
        ...getMsTxInstructions({
            creator: payer,
...
export const getExecuteInstruction = async (
    ...
    const ixList = await Promise.all(
        [...new Array(ixIndex)].map(async (a, i) => {
            ...
            const ixAccount = MsInstruction.fromAccountInfo(ixAccountInfo!, 0)[0] as MsInstructionArgs;
    ...
    const ixKeysList: AccountMeta[] = ixList
        .map(({ pubkey, ixItem }) => {
            ...
            const addAndThreshSig = utils.sha256.hash('global:add_member_and_change_threshold');
            const ixAndThreshDiscriminator = Buffer.from(addAndThreshSig, 'hex');
            const addAndThreshData = Buffer.concat([ixAndThreshDiscriminator.subarray(0, 8)]);
    ...
    const keyIndexMap = ixKeysList.map((a) =>
        keysUnique.findIndex((k) => k.pubkey.toBase58() === a.pubkey.toBase58() && k.isWritable === a.isWritable)
    );
